...
![](Capturas/ebay.png)  
 ...
 # _________________________________________________________________
 ### Kubernetes manual vs Kubernetes nativa  
Comencemos con **kops** , que permite a los usuarios crear, actualizar o eliminar clusters de Kubernetes de alta calidad y de alta producción desde la interfaz de línea de comandos. Mantenido por el equipo central de Kubernetes, la mayor ventaja de kops es que las posibilidades son altas para que se convierta en un método predeterminado de instalación en el futuro. Las características principales de la herramienta incluyen:  
+ Creación automatizada de infraestructura y despliegue de clústeres.  
+ Soporte para características nativas de proveedores de nube.  
+ Soporte para actualizaciones y actualizaciones.  
  
Con kops, uno también puede agregar una variedad de complementos personalizados de Kubernetes . Además de esto, puede agregar contenedores como enlaces y archivos a los nodos a través de un manifiesto de clúster. Kops es actualmente la mejor herramienta para implementar clusters de Kubernetes en los servicios web de Amazon. El proyecto se describe a sí mismo como kubectl para clusters.Tiene comandos para crear clústeres, actualizar sus configuraciones y aplicar cambios. Kops utiliza una configuración declarativa, por lo que es lo suficientemente inteligente como para saber cómo aplicar los cambios de infraestructura a los clústeres existentes. También tiene soporte para tareas operativas de clúster como escalar nodos o escalar horizontalmente el clúster.  
+ Implementa clústeres en las nubes privadas virtuales existentes o cree una nueva VPC desde cero
+ Soporta topologías públicas y privadas.
+ Aprovisiona individuales o múltiples agrupaciones maestras
+ Máquinas bastion configurables para el acceso SSH a nodos de clúster individuales
+ Construido en un modelo de sincronización de estado para pruebas en seco e idempotencia automática
+ Manipulación directa de la infraestructura, o funciona con CloudFormation y Terraform
+ Actualizaciones de cluster rodantes
+ Admite clústeres heterogéneos mediante la creación de múltiples grupos de instancias.   
  
Por otro lado **Amazon Elastic Container Service for Kubernetes** (Amazon EKS) facilita la implementación, la administración y el escalado de aplicaciones en contenedores mediante el uso de Kubernetes en AWS.
Amazon EKS dispone de una certificación de conformidad con Kubernetes, de manera que puede utilizar todos los complementos y las herramientas de los socios y de la comunidad de Kubernetes. Las aplicaciones que se ejecuten en cualquier entorno de Kubernetes estándar son totalmente compatibles y pueden migrarse fácilmente a Amazon EKS. Amazon Elastic Container Service for Kubernetes es un servicio administrado que le permite ejecutar fácilmente Kubernetes en AWS sin necesidad de instalar, operar ni mantener su propio plano de control de Kubernetes.Amazon EKS administra automáticamente la disponibilidad y escalabilidad de los nodos del plano de control de Kubernetes responsables de iniciar y detener contenedores, programar contenedores en máquinas virtuales, almacenar datos de clústeres y otras tareas. Amazon EKS detecta y reemplaza automáticamente nodos de plano de control con errores para cada clúster.

Con Amazon EKS, puede aprovechar el desempeño, la escala, la fiabilidad y la disponibilidad de la plataforma de AWS, así como las integraciones con las redes y los servicios de seguridad de AWS, como Application Load Balancer para la distribución de cargas, IAM para el control del acceso basado en funciones y VPC para redes de pod.Paga 0,20 USD por hora por cada clúster de Amazon EKS que cree. Puede usar un único clúster de Amazon EKS para ejecutar varias aplicaciones mediante el uso de los espacios de nombres de Kubernetes y las políticas de seguridad de IAM.

Solo tiene que pagar por los recursos de AWS (por ejemplo, instancias EC2 o volúmenes de EBS) que cree para los nodos de trabajo de Kubernetes. Solo pagará por lo que consuma y a medida que lo haga: no se requieren pagos mínimos ni compromisos iniciales. 
**Tabla comparativa entre AWS EKS y KOPS**  
![](Capturas/tabla111.PNG)  
![](Capturas/tabla112.PNG)  
A partir de lo anterior, podemos sacar la conclusión que cada una de las soluciones nos ofrece aspectos diferentes y que la decisión de adoptar una de ella depende de lo que requiramos. Ya que una solución nativa en este caso con AWS, nos brinda un servicio en donde AWS no brinda cierto soporte. Además, no aporta una tolerancia a fallas mas alta ya que es AWS que se encargaría de las posibles fallas ya que es quien gestiona gran parte de los servicios. Evidentemente esto al ser un servicio brindado por Amazon tiene un costo a diferencia de KOPS que es gratuito. Claro está que para KOPS la tolerancia a fallas dependerá de la capacidad del experto quien se encargue de gestionarlo. Ya que una de las ventajas en mi opinión de KOPS, es su alta facilidad de personalización. Lo que le brinda al gestor un alto grado de control. Por lo que decir que uno de los dos es mejor que otro es difícil. Todo depende de lo que la empresa necesite y de lo que se prefiera.
